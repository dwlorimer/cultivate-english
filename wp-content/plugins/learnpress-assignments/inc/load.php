	...
	class LP_Addon_Assignment extends LP_Addon {
		...
		public function __construct() {
			...
			if ( $this->_check_version() ) {
				...
				add_filter( 'learn-press/new-section-item', array( $this, 'new_assignment_item' ), 10, 2 );
				...
				add_filter( 'lear-press/filter-user-access-types', array( $this, 'add_filter_access' ) );
		...
		public function add_profile_publicity_fields( $profile ) {
			if ( LP()->settings()->get( 'profile_publicity.assignments' ) === 'yes' ) { ?>
                <li class="form-field">
                    <label for="my-assignments"><?php _e( 'My assignments', 'learnpress-assignment' ); ?></label>
		...
		public function add_profile_tabs( $tabs ) {
			...
			$tabs['assignment'] = array(
				'title'    => __( 'Assignments', 'learnpress-assignment' ),
		...
		public function emails_setting() {
			if ( ! class_exists( 'LP_Settings_Emails_Group' ) ) {
				include_once ABSPATH . '/wp-content/plugins/learnpress/inc/admin/settings/email-groups/class-lp-settings-emails-group.php';
			}
		 ...
		 * @return int|WP_Error
		 */
		public function new_assignment_item( $item, $args ) {
		...
		public static function process_assignment() {
			...
			if ( '' == $user_note ) {
				...
				learn_press_update_assignment_item( $assignment_id, $course_id, $user, 'doing' );
				if ( 'Send' == $pressed_button ) {
					...
					learn_press_update_assignment_item( $assignment_id, $course_id, $user, 'completed' );
				}
			}
