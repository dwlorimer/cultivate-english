...
if ( ! class_exists( 'Thim_Core_Installer' ) ) {
	...
	class Thim_Core_Installer {
		...
		private $theme = array(
			'name'    => 'Education Pack',
			'slug'    => 'education-pack',
			'support' => 'https://thimpress.com/forums/forum/education-pack/'
		);
		...
		public function notice_install() {
			...
			if ( ! education_pack_admin_notice_active( 'dismissible-thim-core-1' ) ) {
				return;
			}

			?>
			<div class="notice notice-success is-dismissible" data-dismissible="dismissible-thim-core-1">
				<h3><?php printf( __( '%s Theme notice!', 'education-pack' ), $theme['name'] ) ?></h3>
				<p>
					<?php printf( __( 'Installed the theme successfully. <a href="%s">Enable Thim Core to start now!</a>', 'education-pack' ), $link ) ?>
				</p>
			</div>
			<?php
		}
		...
		public function add_menu_installer() {
			...
			add_theme_page(
				esc_attr__( 'Thim Core Installer', 'education-pack' ),
				'Thim Core Installer',
				'edit_theme_options',
				$this->slug,
		...
		private function setup_content() {
			...
			<div class="thim-setup-content">
				<div class="header text-center">
					<h1 class="title"><?php esc_html_e( 'Thim Core Installer', 'education-pack' ) ?></h1>
					<div class="sub-title">
						<?php printf( __( 'Welcome to the setup Core for %1$s Theme. Thank you for choosing the %1$s theme!', 'education-pack' ), $theme['name'] ); ?>
					</div>
				</div>
				...
				<?php else: ?>
					...
					<div class="thim-core-active">
						<h2><?php esc_html_e( 'Your site have already installed Thim Core!', 'education-pack' ); ?></h2>

						<div><?php printf( __( '<button data-href="%s" class="thim-button-link button button-primary">Return to Dashboard</button>', 'education-pack' ), admin_url( 'admin.php?page=thim-dashboard' ) ) ?></div>
					</div>
		...
		public function prepare_installation() {
			...
			<div class="install-container text-center">
				<button class="thim-button-link button button-primary" data-href="<?php echo esc_url( $link ) ?>"><?php esc_html_e( 'Install and activate', 'education-pack' ) ?></button>
			</div>
			<?php
		}
		...
		public function step_install() {
			...
			?>
			<h3><?php esc_html_e( 'Installing Thim Core', 'education-pack' ) ?></h3>
			...
			<h3><?php esc_html_e( 'Activating Thim Core', 'education-pack' ) ?></h3>
			<div class="updating-message notice notice-success">
				<p><?php esc_html_e( 'Activating...', 'education-pack' ) ?></p>
			</div>
			<?php
		...
		public function step_activate() {
			...
			if ( $plugin->activate( true ) || $plugin->is_active() ) {
				?>
				<h3><?php esc_html_e( 'Activating Thim Core successfully!', 'education-pack' ) ?></h3>
				<div class="updating-message notice notice-success">
					<p><?php printf( __( 'You are redirecting to %s theme dashboard...', 'education-pack' ), $theme['name'] ) ?></p>
				</div>
				<?php
			...
			?>
			<h3><?php esc_html_e( 'Activating Thim Core failed!', 'education-pack' ) ?></h3>
			<div class="notice notice-error">
				<p><?php esc_html_e( 'Something went wrong!', 'education-pack' ) ?></p>
				<p>
					<button data-href="<?php echo esc_url( $link ) ?>" class="thim-button-link button button-primary"><?php esc_html_e( 'Activate again', 'education-pack' ) ?></button>
				</p>
			</div>
		...
		private function setup_environments() {
			...
			<table class="widefat striped" cellspacing="0">
				<thead>
				<tr>
					<th colspan="2"><?php esc_html_e( 'Configuration Check', 'education-pack' ) ?></th>
				</tr>
				</thead>

				<tbody>
				<tr>
					<td><?php esc_html_e( 'Server Info', 'education-pack' ); ?></td>
					<td><?php echo esc_html( $args['server_info'] ); ?></td>
				</tr>

				<tr>
					<td><?php esc_html_e( 'PHP Version', 'education-pack' ); ?></td>
					<td>
						<?php
						if ( version_compare( $args['php_version'], '5.6', '<' ) ) {
							$link_update_php = 'https://goo.gl/WRBYv3';
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%1$s - We recommend a minimum PHP version of 5.6. See: %2$s', 'education-pack' ), esc_html( $args['php_version'] ), '<a href="' . esc_url( $link_update_php ) . '" target="_blank">' . __( 'How to update your PHP version', 'education-pack' ) . '</a>' ) . '</mark>';
						} else {
				...
				<tr>
					<td><?php esc_html_e( 'cURL version', 'education-pack' ); ?>:</td>
					<td><?php echo esc_html( $args['curl_version'] ) ?></td>
				</tr>

				<tr>
					<td><?php esc_html_e( 'Remote GET', 'education-pack' ); ?></td>
					<td><?php
						if ( $args['remote_get_successful'] ) {
							echo '<mark class="yes"><span class="dashicons dashicons-yes"></span></mark>';
						} else {
							echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . esc_attr__( 'wp_remote_get() failed. Please contact your hosting provider.', 'education-pack' ) . ' ' . esc_html( $args['remote_get_response'] ) . '</mark>';
						} ?>
					</td>
				</tr>

				<tr>
					<td><?php esc_html_e( 'Plugins directory', 'education-pack' ); ?></td>
					<td>
				...
				<tr>
					<td><?php esc_html_e( 'Plugins directory writable', 'education-pack' ); ?></td>
			...
			<div class="notice notice-error">
				<p><?php printf( __( 'Please follow those instructions above to make sure your server is ready to use %s theme. If you need assistance, please get our support <a href="%s" target="_blank">here</a>.', 'education-pack' ), $theme['name'], $theme['support'] ) ?></p>
			</div>
			<?php
		}
	...
	private function setup_header() {
		...
		<head>
			...
			$title = 'title';
			echo '<' . $title . '>';
			echo printf( __( 'Thim Core Installer &lsaquo; %s', 'education-pack' ), $theme['name'] );
			echo '</' . $title . '>';
		...
		<div class="footer text-center">
			<div class="brand">
				<?php esc_html_e( '&copy; ThimPress 2017. All rights reserved. Powered by WordPress.', 'education-pack' ) ?>
			</div>
		</div>
		</body>
		<?php
